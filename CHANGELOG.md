...
- `historicnames` on city records, and `historicnames` as a searchable `search_cities()` attribute, in the same per-language shape admin1 and admin2 records have. 967 of the 34,078 cities at the default threshold have one. This is where renamed places now live: "Kiev" is flagged historic upstream, so `search_cities('Kiev')` returns nothing and `search_cities('Kiev', 'historicnames', contains_search=False)` returns Kyiv.
- `get_city_names(city, languages=None, historic=False)`, the city counterpart of `get_admin1_names()`: `name` first, then the selected language buckets, deduplicated. Untagged names and abbreviations are always included, an unknown language code contributes nothing rather than raising.
- A `py.typed` marker, so type checkers use the annotations that are already there instead of treating the package as untyped. Downstream projects no longer need `ignore_missing_imports` for `geonamescache`.
...
- **Breaking:** city `alternatenames` is a `dict[str, list[str]]` keyed by ISO-639 language code instead of a flat `list[str]`, built from `alternateNamesV2.txt` rather than the untagged `alternatenames` column of the cities dumps. That column mixes in GeoNames' own ASCII romanisation of every language a place has a name in, which is what made the list unusable for matching: Rotterdam's 43 entries included 25 romanisations — "Roterdam", "Ratehrdam", "loteleudam", "rwtrdm" — with nothing marking them as such, and the romanisations collide across languages the same way they do for divisions. Only the languages of the city's own country plus English are kept, along with untagged names and abbreviations; reference codes (`link`, `wkdt`, `post`, `iata`, `icao`, `faac`, `unlc`, `tcid`, `phon`, `piny`) are dropped. Across the default dataset the name count falls from 349,573 to 111,237, and the bundled city data from 35 MB to 22 MB — the wheel shrinks from 37.3 MB to 24.7 MB.
- An exact, case insensitive `search_cities()` is answered from an index keyed by country code then casefolded value, built once per attribute, the same trade `get_cities_by_names()` already makes for the primary name. The scanning paths — contains search, or case sensitive — now iterate only the given country's records when a `countrycode` is passed, instead of the whole dataset with a per-record country test. **Results from the indexed path come out in country order rather than geonameid order**; nothing else about them changes.
- `bin/cities.py` builds all four population thresholds in one run, so the ~780 MB `alternateNamesV2.txt` is streamed once for the union of their ids rather than once per dump. It now also requires `countryInfo.txt`, which `./bin/download_data.py` already fetches.
